...
**They are four apart in the chain.** `chain.ts` nests eleven global middlewares with `Layer.flatMap`, and registration order is nesting order. `cors` is fifth and `csrf` is ninth; between them sit `timing`, `timeout` and `language`. So what a response carries depends on where each one is: a cors preflight is answered before `timing` ever runs and leaves without a `Server-Timing` header, while a csrf 403 is produced inside the eight outside it and carries everything, including the language cookie. One layer cannot hold two positions. A merged module would either export two layers anyway — which is what it already has — or collapse the spacing and change four responses.
